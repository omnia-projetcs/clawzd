...
class HeartMuLaPipeline:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if audio_guide or kwargs.get("audio_guide2"):
            print("[HeartMuLa] Warning: Native reference audio is not supported in this version. Falling back to text prompt guidance.")
    ...
    def generate_with_callback(
        ...
        **kwargs,
        ...
        """
        Generate audio using model inputs and callback, saving the result directly to save_path.
        This provides a high-level compatibility layer for Clawzd tools.
        """
        ...
        # Handle ref audio warning or fallback
        if ref_audio:
            print("[HeartMuLa] Warning: Native reference audio is not supported in this version. Falling back to text prompt guidance.")
            if "style transfer" not in tags:
                tags = f"{tags}, style transfer"
        ...
        # Save to save_path
        import torchaudio
        torchaudio.save(save_path, wav.to(torch.float32).cpu(), self.sample_rate)
        return save_path
